...
if($stmt->rowCount()>0){
 ...
 if($results['fee_status']==='paid'){
    //checking if welfare officer has been votted
    $check = "SELECT * FROM users_7th WHERE  matricule=:matricule";
    $stmt = $pdo->prepare($check);
    $stmt->bindParam(":matricule",$matricule);
    $stmt->execute();
    if($stmt->rowCount()>0){
        ...
        //vote welfare officer
        $sql = "INSERT into users_7th (matricule,candidate) values(:matricule,:candidate)";
...
// $matricule = $_REQUEST['txtmat'];
// $candi = $_REQUEST['txtCand'];
// //checking if General secretory has been votted
// $check = "SELECT * FROM users_7th WHERE matricule=:matricule";
// $stmt = $pdo->prepare($check);
// $stmt->bindParam(":matricule",$matricule);
// $stmt->execute();
