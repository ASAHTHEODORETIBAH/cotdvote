...
if($stmt->rowCount()>0){
 ...
 if($results['fee_status']==='paid'){
    //checking if General Organiser has been votted
    $check = "SELECT * FROM users_3rd WHERE  matricule=:matricule";
    $stmt = $pdo->prepare($check);
    $stmt->bindParam(":matricule",$matricule);
    $stmt->execute();
    if($stmt->rowCount()>0){
        ...
        //vote General Organiser
        $sql = "INSERT into users_3rd (matricule,candidate) values(:matricule,:candidate)";
