...
if($stmt->rowCount()>0){
 ...
 if($results['fee_status']==='paid'){
    //checking if General secretary has been votted
    $check = "SELECT * FROM users_2nd WHERE  matricule=:matricule";
    $stmt = $pdo->prepare($check);
    $stmt->bindParam(":matricule",$matricule);
    $stmt->execute();
    if($stmt->rowCount()>0){
        ...
        $sql = "INSERT into users_2nd(matricule,candidate) values(:matricule,:candidate)";
