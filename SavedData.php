...
if($stmt->rowCount()>0){
 ...
 if($results['fee_status']==='paid'){
    //checking if president has been votted
    $check = "SELECT * FROM users WHERE  matricule=:matricule";
    $stmt = $pdo->prepare($check);
    $stmt->bindParam(":matricule",$matricule);
    $stmt->execute();
    if($stmt->rowCount()>0){
        ...
        //vote president
        $sql = "INSERT into users(matricule,candidate) values(:matricule,:candidate)";
