...
if($stmt->rowCount()>0){
 ...
 if($results['fee_status']==='paid'){
    //checking if Teasurer has been votted
    $check = "SELECT * FROM users_4th WHERE  matricule=:matricule";
    $stmt = $pdo->prepare($check);
    $stmt->bindParam(":matricule",$matricule);
    $stmt->execute();
    if($stmt->rowCount()>0){
        ...
        //vote Teasurer
        $sql = "INSERT into users_4th (matricule,candidate) values(:matricule,:candidate)";
