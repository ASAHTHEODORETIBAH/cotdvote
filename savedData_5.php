...
if($stmt->rowCount()>0){
 ...
 if($results['fee_status']==='paid'){
    //checking if sports coordinator has been votted
    $check = "SELECT * FROM users_5th WHERE  matricule=:matricule";
    $stmt = $pdo->prepare($check);
    $stmt->bindParam(":matricule",$matricule);
    $stmt->execute();
    if($stmt->rowCount()>0){
        ...
        //vote sports coordinator
        $sql = "INSERT into users_5th (matricule,candidate) values(:matricule,:candidate)";
