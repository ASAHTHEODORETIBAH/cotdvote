...
if($stmt->rowCount()>0){
 ...
 if($results['fee_status']==='paid'){
    //checking if Academic Officer has been votted
    $check = "SELECT * FROM users_8th WHERE  matricule=:matricule";
    $stmt = $pdo->prepare($check);
    $stmt->bindParam(":matricule",$matricule);
    $stmt->execute();
    if($stmt->rowCount()>0){
        ...
        //vote Academic Officer
        $sql = "INSERT into users_8th (matricule,candidate) values(:matricule,:candidate)";
