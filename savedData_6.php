...
if($stmt->rowCount()>0){
 ...
 if($results['fee_status']==='paid'){
    //checking if Financial secretary has been votted
    $check = "SELECT * FROM users_6th WHERE  matricule=:matricule";
    $stmt = $pdo->prepare($check);
    $stmt->bindParam(":matricule",$matricule);
    $stmt->execute();
    if($stmt->rowCount()>0){
        ...
        $sql = "INSERT into users_6th (matricule,candidate) values(:matricule,:candidate)";
